...
def brute_optimize(
    ...
    optimize_args_tuple = (equation_function, loss_function, X_ndarray, y_df.values, verbose)  # , X_col_names)
...
def custom_objective_function(parameters_to_estimate_1darray, *args_tuple):
    ...
    equation_function, loss_function, fixed_parameters_ndarray, y_actual, verbose = args_tuple
    y_estimate = equation_function(parameters_to_estimate_1darray, fixed_parameters_ndarray)
    loss_score = loss_function(y_actual, y_estimate)  # , equation_function, parameters_to_estimate_1darray)
    if verbose:
            ...
            for n_local_searches in range(len(local_search_df)):
                ...
                for col_name in list(X_df.columns):
                    ...
                    parameter_search_range_list.append(
                        slice(np.round(local_val - m, 5), np.round(local_val + m + step, 5), np.round(step, 5))
                    )
...
for y in [["BASAL"]]:  # [["BASAL", "log_BASAL"]]:  #, ["ISF", "log_ISF"], ["CIR", "log_CIR"]]:
    ...
    for combo, ac in enumerate(all_combos):
        ...
        for i, j in kf.split(X_train):
            ...
            y_predict = linear_regression_equation(top_result.loc[0, X_cols].values, X_df_val.values)
            if "log" in y_lin_log:
